class Viajero:
    ...
    def __eq__(self, otro):       #por izquierda
        if type( otro ) == int:
            return self.__millasAcum == otro
        
    def __eq__(self, otro):       #por derecha
        if type( otro ) == int:
            return otro == self.__millasAcum

    def __ge__ (self, otro): #comparacion del canje
        if type( otro ) == int:
            return self.__millasAcum >= otro    

    def __sub__ (self, otro): #canje
        if type( otro ) == int:
